...
def user_create(request):
    ...
    try:
        ...
        logger.info(f"管理员 {request.user.username} 尝试创建用户: {username}")
        
        # 检查用户名是否已存在
        if User.objects.filter(username=username).exists():
            logger.warning(f"用户创建失败: {username} - 用户名已存在")
            return JsonResponse({'status': 'error', 'message': '用户名已存在'}, status=400)
        
        # 检查密码是否为空
        password = data.get('password')
        if not password:
            logger.warning(f"用户创建失败: {username} - 创建用户时密码不能为空")
        ...
        logger.error(f"用户创建失败: {username} - {str(e)}")
...
def user_update(request, user_id):
    ...
    try:
        ...
        username = data.get('username')
        logger.info(f"管理员 {request.user.username} 尝试更新用户信息: {user.username} -> {username}")
        
        # 检查新用户名是否已存在（排除当前用户）
        if User.objects.exclude(pk=user_id).filter(username=username).exists():
            logger.warning(f"用户信息更新失败: {username} - 用户名已存在")
            return JsonResponse({'status': 'error', 'message': '用户名已存在'}, status=400)
...
def change_password(request, user_id):
    ...
    if not request.user.is_superuser and request.user.id != user_id:
        logger.warning(f"密码修改失败: {request.user.username} 尝试修改用户 {user_id} 的密码但没有权限")
        return JsonResponse({'status': 'error', 'message': '没有权限'}, status=403)
    
    try:
        data = json.loads(request.body)
        user = get_object_or_404(User, pk=user_id)
        logger.info(f"用户 {request.user.username} 尝试修改密码: {user.username}")
        
        # 检查新密码是否为空
        new_password = data.get('new_password')
        if not new_password:
            logger.warning(f"密码修改失败: {user.username} - 新密码不能为空")
        ...
        logger.error(f"密码修改失败: {user.username} - {str(e)}")
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
